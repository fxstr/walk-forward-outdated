...
export default class HighChartsExporter {
  constructor() {
    _defineProperty(this, "mainChartIdentifier", 'main-chart');

    _defineProperty(this, "series", []);

    _defineProperty(this, "charts", new Map());

    _defineProperty(this, "chartIdCounter", 0);
  }
  ...
  async export(dataSeries, basePath, name) {
    ...
    this.basePath = basePath; // dataSeries does not contain a name, but e.g. instrument (extended class) does
  ...
  exportOHLC() {
    const ohlcDataArray = this.dataSeries.data.map(row => // Use array (as object doesn't work for > 1000 rows, see
    // https://api.highcharts.com/highstock/series.ohlc.data)
    [row.get('date').getTime(), row.get('open'), row.get('high'), row.get('low'), row.get('close')]);
    this.series.push({
      type: 'ohlc',
      name: this.name,
      data: ohlcDataArray,
  ...
  createMainChart() {
    ...
    this.charts.push(mainConfig); // Smart: If we look up a chart with a missing config (=undefined) later, mainChart will
    // be used.
  ...
  createDataForColumn(columnKey) {
    ...
    const chartViewConfig = this.dataSeries.viewConfig && this.dataSeries.viewConfig.chart && this.dataSeries.viewConfig.chart.get(columnKey);
    let seriesConfig;
    let chartConfig; // There's a chart config for this column
    ...
    seriesConfig = seriesConfig || {}; // Handle charts first: We need its ID to reference it by series
    // Chart may appear in multiple series configs. Only add it to this.charts once.

    let chart; // console.debug('chartConfig is %o', chartConfig);
    ...
    if (this.mappedCharts.has(chartConfig)) {
      ...
      chart = _objectSpread({}, this.createDefaultChartConfig(columnKey), spreadableChartConfig, {
      ...
      this.charts.push(chart); // Never add undefined to mappedCharts as we want an own chart for all series that do
    ...
    const highchartsSeries = _objectSpread({}, seriesConfig, {
  ...
  createSeriesArrayFromColumn(columnKey) {
    return this.dataSeries.data // Remove all empty rows
    .filter(row => row.get(columnKey) !== undefined) // Object -> Array
  ...
  updateHeights() {
    const relativeSpace = 0.1; // Only get totalChartHeight once: afterwards we update chart.height to a string (e.g.
    // '50%', adding up won't work any more); add 0.05 for spacing between charts

    const totalHeight = this.getTotalChartHeight() + this.charts.length * relativeSpace; // Update heights (from relations to percentages); previous value is used as top position
    // of current chart
...
//# sourceMappingURL=HighChartsExporter.mjs.map