...
export default class BacktestInstruments extends AwaitingEventEmitter {
  ...
  constructor(generatorFunction, startDate, endDate) {
    ...
    _defineProperty(this, "instruments", new Map());
  ...
  async run() {
    ...
    for await (const intervalData of this.generatorFunction()) {
      ...
      intervalData.forEach(dataSet => checkInstrumentData(dataSet)); // Only emit if current startDate < intervalData < endDate

      const currentDate = intervalData[0].get('date');
      if (this.startDate && this.startDate.getTime() > currentDate.getTime()) continue;
      if (this.endDate && this.endDate.getTime() < currentDate.getTime()) continue; // Open: Create instrument, add open and date

      const openedInstruments = await this.openInstruments(intervalData);
      log.info('%s Emit open', formatDate(currentDate));
      await this.emit('open', openedInstruments); // Close
  ...
  async openInstruments(instrumentsData) {
    return Promise.all(instrumentsData.map(async instrumentData => {
      ...
      await instrument.add(new Map([['date', instrumentData.get('date')], ['open', instrumentData.get('open')]]));
      return instrument;
    }));
  }
  /**
  ...
  async closeInstruments(instrumentsData) {
    return Promise.all(instrumentsData.map(async instrumentData => {
      const instrument = await this.getOrCreateInstrument(instrumentData.get('instrument')); // Delete fields that were already added to instrument from (a clone of) instrumentData
  ...
  */
  ...
  async getOrCreateInstrument(name) {
    ...
    if (existingInstrument) return existingInstrument;else {
...
//# sourceMappingURL=BacktestInstruments.mjs.map