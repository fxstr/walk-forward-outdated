...
export default class Backtest {
  constructor() {
    _defineProperty(this, "strategyFunction", void 0);

    _defineProperty(this, "dataSource", void 0);

    _defineProperty(this, "performanceIndicators", []);

    _defineProperty(this, "instances", new Map());

    _defineProperty(this, "optimization", new Optimization());
  }
  ...
  */
  ...
  async run() {
    ...
    const parameterSets = hasOptimizations ? this.optimization.generateParameterSets() : [undefined];
    log.info(`Run backtest for ${parameterSets.length} parameter sets.`);

    for (const parameterSet of parameterSets) {
      log.info(`Current parameter set is ${parameterSet}.`); // Define INSIDE of parameter loop; as a BacktestInstrument emits open/close events,
      // it must be re-instantiated every single time we run a backtest instance. Caching, if
      // needed, is done within the data source.

      const instruments = new BacktestInstruments(this.dataSource.generate.bind(this.dataSource), this.configuration.get('startDate'), this.configuration.get('endDate'));
      ...
      const instance = new BacktestInstance(instruments, parameterizedStrategyRunner, this.configuration);
      await instance.run();
...
//# sourceMappingURL=Backtest.mjs.map