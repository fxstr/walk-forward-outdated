...
const {
  debug
} = logger('WalkForward:DataSeriesExporter');
/**
...
export default class DataSeriesExporter {
  ...
  reformatData(dataSeries) {
    ...
    const result = [[]]; // Create head row

    for (const [key, value] of head) {
      let headField = value.description || key; // We advise users to use Symbols as col keys; be sure to give a meaningful warning
    ...
    for (const dataRow of dataSeries.data) {
      ...
      for (const [col] of head) {
        let content = dataRow.get(col); // Make sure we output 0 (but not undefined)
...
//# sourceMappingURL=DataSeriesExporter.mjs.map