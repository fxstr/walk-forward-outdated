...
export default class DataSeries {
  constructor() {
    _defineProperty(this, "internalData", []);
  ...
  */
  add(data) {
    log.debug('Add data %o', data); // If an object (which is not a map) is passed, try to convert it to a Map
    ...
    this.addColumns(data); // Clone data or original data is modified when set() is used which adds a new item to the
    // Map
    ...
    for (const [key, value] of data) {
      ...
      if (headRow.has(key)) {
        throw new Error(`DataSeries: Existing values cannot be overwritten by using the
					set method; tried to overwrite %o with %o, is already %o`, key, value, headRow.get(key));
      }
  ...
  */
  ...
  head(itemCount = 1, startIndex = 0) {
    ...
    const result = this.internalData // Don't use negative numbers, they will slice from the end!
    .slice(Math.max(0, len - itemCount - startIndex), Math.max(0, len - startIndex)).reverse();
  ...
  */
  ...
  tail(itemCount = 1, startIndex = 0) {
    const result = this.internalData.slice(startIndex, itemCount + startIndex);
    return itemCount === 1 ? result[0] : result;
  }
  /**
...
//# sourceMappingURL=DataSeries.mjs.map