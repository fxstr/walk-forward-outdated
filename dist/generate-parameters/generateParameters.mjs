...
function generateLogarithmicParameters(from, to, steps, logBase = goldenRatio) {
  ...
  const originalNumbers = Array.from(new Array(steps)).map((item, index) => logBase ** index); // Reduce (or expand) all steps by a ratio

  const reductionRatio = (to - from) / (originalNumbers[originalNumbers.length - 1] - originalNumbers[0]);
  const result = originalNumbers.map(item => // Deduct reduction ratio because the first orignalNumber is 1, not 0
  from + reductionRatio * item - reductionRatio);
  return result;
}

export { generateLogarithmicParameters };
//# sourceMappingURL=generateParameters.mjs.map