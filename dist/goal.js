import Backtest, { run, rejectOnFalse, runThrough, TransformableDataSeries, instrumentType } from './src/backtest/Backtest';
import BacktestCSVSource from './src/csv-reader/CSVReader';
import SMA from './src/indicators/SMA'; // Backtest exposes:
// - account
// - (orders)
// - positions
// - instruments
...
class Algorithm {} // Monthly rebalanncing
...
class RunMonthly extends Algorithm {
  onClose(orders, instrument) {
    ...
    this.latestDate = now; // If false is returned, strategy is halted; else it continues
...
function serial(strategies, data) {
  ...
  data.forEach(datum => {
    datum.on('data', item => {
      ...
      if (!result) return;else callNextStrategy();
    });
  });
...
(async () => {
  ...
  const backtest = new Backtest(); // First param: Use file name (without .csv ending) as the instrument's name
  // Second param: Read all files in the current folder that end with "-eod.csv"

  backtest.setDataSource(new BacktestCSVSource(name => name.substr(0, -4), ['*-eod.csv'])); // name, [from, to], steps; backtest will be run with 100 variations
  ...
  backtest.setStrategies(params => {
    return runThrough( // Use a simple SMA strategy; buy instruments for the same amount
    rejectOnFalse(new RunMonthly(), // Before buying new instruments, reduce the amount of the ones you're holding
    new RebalancePositions(), new EqualPositionSize()), // Rebalance the open positions every month
    rejectOnFalse(new SMAAlgo('close', params('fastSMA'), params('slowSMA')), new EqualPositionSize()));
  }); // Reads from stream and runs one stream per optimization; accepts start and end date

  await backtest.run(new Date(2010, 0, 1)); // Writes JSON for all instruments (setStream) and result1/2 (setResults) to file system
  // True for zip/folder (?)
