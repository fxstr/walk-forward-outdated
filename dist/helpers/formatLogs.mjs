...
function formatOrders(orders) {
  return Array.from(orders.entries()).map(([instrument, order]) => `${instrument.name}:${order.size}`).join(', ') || '—';
}
/**
...
function formatPositions(positionEntry) {
  return Array.from(positionEntry.entries()).filter(([key]) => key !== 'date' && key !== 'type') // t stands for total
  .map(([instrument, position]) => `${instrument.name}:${position.size}@${position.value}t`).join(', ') || '—';
}

export { formatPositions, formatDate, formatInstruments, formatOrders };
//# sourceMappingURL=formatLogs.mjs.map