...
import dataSortFunction from '../../helpers/dataSortFunction.mjs';
...
export default class BacktestCSVSource {
  ...
  constructor(csvSource) {
    _defineProperty(this, "csvReadingPromise", false);
  ...
  async *generate() {
    await this.setupData(); // eslint-disable-next-line no-unused-vars
  ...
  async setupData() {
    ...
    if (!this.csvReadingPromise) {
      ...
      const csvData = await this.csvReadingPromise; // Sort by date first so that all entries in the map grouped by date are
      // chronologically sorted

      const sortedCSVData = csvData.slice(0).sort(dataSortFunction);
      this.csvDataGroupedByDate = groupArrayByValue(sortedCSVData, // Make sure we use a Number (and not a native date) to group items by as a Date
...
//# sourceMappingURL=BacktestCSVSource.mjs.map