...
const {
  debug,
  info
} = logger('WalkForward:BacktestTest');
...
class SMAAlgo extends Algorithm {
  ...
  constructor(field, fastSMA, slowSMA) {
    ...
    _defineProperty(this, "fastSMAKey", 'fastSma');

    _defineProperty(this, "slowSMAKey", 'slowSma');

    debug('SMAAlgo: Init with %s %d %d, is %o', field, fastSMA, slowSMA, this);
    this.field = field; // Params are floats – convert them to ints
  ...
  handleNewInstrument(instrument) {
    debug('SMAAlgo: Instrument %o added; fast is %d, slow %d', instrument.name, this.fastSMALength, this.slowSMALength);
    instrument.addTransformer([this.field], new Sma(this.fastSMALength), this.fastSMAKey);
    instrument.addTransformer([this.field], new Sma(this.slowSMALength), this.slowSMAKey);
  }
  ...
  handleClose(orders, instruments) {
    ...
    for (const instrument of instruments) {
      debug('onClose, orders are %o, instrument is %o', orders, instrument); // We need at least 2 data sets to look back
      ...
      info('%s: fast %d slow %d prevFast %d prevSlow %d', instrument.name, fast, slow, prevFast, prevSlow); // If any value is missing, return

      if ([slow, fast, prevSlow, prevFast].includes(undefined)) continue; // Cross-over: Buy
      else if (fast > slow && prevFast <= prevSlow) {
          debug('SMA: Create order for %o', instrument.name);
          newOrders.set(instrument, {
            size: 1
          });
        } // Cross-under: Short
        else if (fast < slow && prevFast >= prevSlow) {
...
class StochIndicators extends Algorithm {
  ...
  constructor(...args) {
    ...
    _defineProperty(this, "stochKKey", 'stoch_K');

    _defineProperty(this, "stochDKey", 'stoch_D');

    this.periods = args;
  }

  handleNewInstrument(instrument) {
    instrument.addTransformer(['high', 'low', 'close'], new Stoch(this.periods[0], this.periods[1], this.periods[2]), {
      stoch_k: this.stochKKey,
      stoch_d: this.stochDKey
    });
  }
...
class EqualPositionSize extends Algorithm {
  async handleClose(orders) {
    ...
    await new Promise(resolve => setTimeout(resolve), Math.random() * 20); // Don't rebalance daily – only calculate sizes when orders are available

    if (!orders.size) return orders; // Map with key: instrument, value: 1 for long or -1 for short for all orders *AND*
    ...
    const moneyAvailable = this.backtest.accounts.head().get('cash') + this.backtest.accounts.head().get('invested');
    info('Accounts is %o', this.backtest.accounts.head()); // Money that is available per position (every position gets the same amount)
    // 0.9: Keep some cash/prices may change from close to open

    const moneyPerPosition = moneyAvailable / newPositions.size * 0.9;
    info('Money available is %d, per position %d', moneyAvailable, moneyPerPosition); // Get target positions; key: instrument, value: size
    ...
    for (const [instrument, direction] of newPositions) {
      ...
      info('%s. Target position is %d, close is %d', instrument.name, size, close);
    ...
    for (const [instrument, targetSize] of targetPositions) {
      const previousSize = this.getCurrentPositions().has(instrument) ? this.getCurrentPositions().get(instrument).size : 0; // console.log('previousSize', previousSize, targetSize);

      const size = targetSize - previousSize;
      info('%s: Target size %d, previous size %d, order size %d', instrument.name, targetSize, previousSize, size);
      newOrders.set(instrument, {
        size
      });
...
async function runTest() {
  ...
  const config = new Map([['cash', 10000], // CAREFUL: Use strings as constructor argument because we do the same when reading data
  // from our CSVs. Strings are GMT-based while numbers are based on the local time zone!
  ['startDate', new Date('2018-01-01')], ['endDate', new Date('2018-01-20')]]);
  backtest.setConfiguration(config);
  backtest.setStrategy(params => [new StochIndicators(3, 1, 2), new SMAAlgo('close', params.get('fastSMA'), params.get('slowSMA')), // new SMAAlgo('close', 2, 3),
...
test('outputs correct results', async t => {
  clearDirectory();
  await runTest();
  const result = compareDir.compareSync(path.join(__dirname, 'test-data/output'), path.join(__dirname, 'test-data/expected-output'), {
    compareContent: true
  });
  console.log('result', result);
  t.is(result.differences, 0);
});
//# sourceMappingURL=backtest.require.mjs.map